...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train the Module B CandidateTokenModulator using normal-only "
            "reference tokens."
        )
    )
...
def main() -> None:
    args = parse_args()
    set_seed(args.seed)
    print("[Module B] CandidateTokenModulator normal-only training", flush=True)
    ...
    if adapter is None:
        raise RuntimeError("CandidateTokenModulator is not present in the model.")
    adapter.float().train()
    ...
    save_checkpoint(
        args.output_dir / "ctm_epoch00_untrained.pt",
        initial_state,
        args,
        epoch=0,
    ...
    save_checkpoint(
        args.output_dir / f"ctm_epoch{args.epochs:02d}_trained.pt",
    ...
    with (args.output_dir / "training_report.json").open(
        ...
        json.dump(
            {
                "protocol": "Module B CandidateTokenModulator normal-only training",
                "seed": args.seed,
                "epochs": args.epochs,
                "num_reference_images": len(image_paths),
                "num_tokens": int(tokens.shape[0]),
    ...
    print(f"[Module B] training complete: {args.output_dir}", flush=True)

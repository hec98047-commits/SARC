...
def parse_args():
    parser = argparse.ArgumentParser(description="Few-shot PGCRE-FGCLIP wrapper using sampled normal calibration only.")
    parser.add_argument("--dataset", choices=["mvtec", "visa"], required=True)
    parser.add_argument("--data_root", type=Path, required=True)
    parser.add_argument("--model_path", type=Path, required=True)
    parser.add_argument("--mg_model_path", type=Path, required=True)
    ...
    parser.add_argument(
        "--disable_mg_branch",
        action="store_true",
        help="Disable the candidate-guided MG/B branch for a matched B-off control.",
    )
    parser.add_argument(
        "--disable_ms_branch",
        action="store_true",
        help="Disable local multi-scale tiles so Module C is excluded.",
    )
    parser.add_argument(
        "--mg_fusion_mode",
    ...
    parser.add_argument("--nrd_ref_selection", choices=["uniform", "fps"], default="uniform")
...
def main():
    ...
    print(
        "[Module A] semantic prompt routing: "
        f"{'enabled' if module_a_enabled else 'disabled'}"
    )
    print(
        "[Module B] candidate-guided response decoupling: "
        f"{'enabled' if module_b_enabled else 'disabled'} "
        f"(CTM={'loaded' if args.token_modulator_checkpoint is not None else 'not loaded'}, "
        f"NRD={'enabled' if args.enable_nrd else 'disabled'})"
    )
    print(
        "[Module C] local detail reasoning and residual fusion: "
        f"{'enabled' if module_c_enabled else 'disabled'}"
    )
    ...
    import run_ms_fb_mg_fgclip as pgcre
    ...
    def load_model_with_token_modulator(model_path):
        ...
        if (
            ...
            print(f"[Module B] CandidateTokenModulator loaded: {args.token_modulator_checkpoint}")
    ...
    def infer_ms_fb_mg_with_nrs(image, baseline_ctx, mg_ctx, baseline_stats, mg_stats, runtime_args, **kwargs):
        ...
        if (args.enable_nrs and not has_nrs_memory) or (args.enable_nrd and not has_nrd_memory):
            if not getattr(runtime_args, "_fewshot_nrs_warning_printed", False):
                print(f"[FewShot][NRS/NRD][WARN] Missing class/memory for image; reference scoring skipped. class={class_name}")
        ...
        if args.enable_nrd:
            ...
            if args.nrd_zscore != "image":
                print("[FewShot][NRD][WARN] nrd_zscore=class is not implemented in low-storage mode; using image.")
    ...
    print(f"[FewShot] Ours PGCRE few-shot ready: {args.output_dir / 'eval_metrics.json'}")
    if args.enable_nrd:
        print("[Module B] NRD ready; reference patches remain in memory.")

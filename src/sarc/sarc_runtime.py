...
from fgclip_ad.sarc_ops import (  # noqa: E402
...
from lec.ncrs_selection import (  # noqa: E402
    calibrate_risk_threshold,
    select_response as ncrs_select_response,
)
from lec.response_calibration import calibrate_response  # noqa: E402
from lec.reliability_fusion import (  # noqa: E402
    compute_reliability_weight,
    reliability_aware_positive_fusion,
)
from lec.safe_response_selection import safe_select_response  # noqa: E402
...
SARC_STAGE_METHODS = ("fgclip_baseline", "prompt", "prompt_mask", "prompt_mask_ms", "full")
LEGACY_METHODS = (
    "baseline",
    "fgclip_baseline",
...
ALL_METHODS = SARC_STAGE_METHODS + LEGACY_METHODS
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Internal SARC inference engine for SP, ARC, and LEC.",
        epilog=(
            "Example (do not run on private data unless you explicitly provide the paths):\n"
            "python sarc_runtime.py ^\n"
            "  --dataset visa ^\n"
    ...
    parser.add_argument("--mg_model_path", type=Path, default=REPO_ROOT / "models" / "SARC")
...
def resolve_ablation_flags(args):
    if args.method in SARC_STAGE_METHODS and (
        getattr(args, "_user_requested_fg", False) or getattr(args, "_user_requested_foreground_mode", False)
    ):
        warnings.warn("Foreground processing is disabled for this SARC stage configuration.")
    ...
    if args.method_name:
        args.resolved_method_name = args.method_name
    elif args.method in SARC_STAGE_METHODS:
        args.resolved_method_name = args.method
    else:
...
def infer_ms_fb_mg(image, baseline_ctx, mg_ctx, baseline_stats, mg_stats, args, ncrs_tau_risk=None, local_ctx=None, local_stats=None):
    use_hpmr_mode = args.method in SARC_STAGE_METHODS

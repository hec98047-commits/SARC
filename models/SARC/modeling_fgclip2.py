...
try:
    from .vision_convnext import Fgclip2ConvNeXtVisionPipeline
except ImportError:
    Fgclip2ConvNeXtVisionPipeline = None
...
def build_mg_attention_bias(
        mg_patch_mask: Optional[torch.Tensor],
        batch_size: int,
        seq_length: int,
        dtype: torch.dtype,
        device: torch.device,
        mg_mask_threshold: float = 0.5,
        mg_neg_bias: float = -1e4,
        attention_bias_eta: float = 1.0,
) -> Optional[torch.Tensor]:
    """
    Build the symmetric candidate/background attention bias from Eq. (8).

    Args:
        mg_patch_mask:
            shape [B, L], where L == seq_length.
            1 means anomaly / important patch, 0 means background / suppress.
        Returns:
            shape [B, 1, L, L], broadcast over attention heads.
    """
    if mg_patch_mask is None:
        return None

    if mg_patch_mask.ndim != 2:
        raise ValueError(
            f"`mg_patch_mask` must be 2D [B, L], got shape={tuple(mg_patch_mask.shape)}"
        )

    if mg_patch_mask.shape[0] != batch_size:
        raise ValueError(
            f"`mg_patch_mask` batch mismatch: got {mg_patch_mask.shape[0]}, expected {batch_size}"
        )

    if mg_patch_mask.shape[1] != seq_length:
        raise ValueError(
            f"`mg_patch_mask` length mismatch: got {mg_patch_mask.shape[1]}, expected {seq_length}"
        )

    mg_patch_mask = mg_patch_mask.to(device=device, dtype=torch.float32)

    labels = mg_patch_mask >= float(mg_mask_threshold)
    cross_region = labels[:, :, None] != labels[:, None, :]
    bias = -float(attention_bias_eta) * cross_region.to(dtype=torch.float32)
    bias = bias[:, None, :, :]  # [B, 1, L, L]
    return bias.to(dtype=dtype)


class CandidateTokenModulator(nn.Module):
    """Shared D -> D/4 -> D bottleneck used by Eq. (7)."""

    def __init__(self, hidden_size: int):
        super().__init__()
        bottleneck_size = max(1, int(hidden_size) // 4)
        self.down = nn.Linear(hidden_size, bottleneck_size)
        self.activation = nn.GELU()
        self.up = nn.Linear(bottleneck_size, hidden_size)
        # Preserve the pretrained representation before adapter training.
...
class Fgclip2Attention(nn.Module):
    ...
    def forward(
            ...
            mg_patch_mask: Optional[torch.Tensor] = None,
            mg_mask_threshold: float = 0.5,
            mg_neg_bias: float = -1e4,
            attention_bias_eta: float = 1.0,
            **kwargs,
        ...
        # Build MGViT-style anomaly bias on key dimension.
        # Since FGCLIP2 vision encoder has no CLS token, we adapt the mask
        # by biasing all queries to attend more to anomaly patches.
        mg_bias = build_mg_attention_bias(
        ...
        if mg_bias is not None:
            if merged_attention_mask is None:
                merged_attention_mask = mg_bias
            else:
                merged_attention_mask = merged_attention_mask + mg_bias

        # MG bias here is implemented through additive attention mask.
        # To avoid compatibility issues with flash attention mask formats,
        # we switch to eager path when mg_patch_mask is used.
        attention_interface: Callable = eager_attention_forward
        if mg_patch_mask is None and self.config._attn_implementation != "eager":
...
class Fgclip2Encoder(nn.Module):
    ...
    def __init__(self, config: Fgclip2Config):
        ...
        self.token_modulator = (
            CandidateTokenModulator(config.hidden_size)
            if isinstance(config, Fgclip2VisionConfig)
            else None
        )
        self.token_modulation_enabled = bool(getattr(config, "token_modulation_enabled", False))
        self.token_modulation_lambda = float(getattr(config, "token_modulation_lambda", 0.1))
        self.token_modulation_start_layer = int(getattr(config, "token_modulation_start_layer", 3))
        self.token_modulation_end_layer = int(
            getattr(config, "token_modulation_end_layer", config.num_hidden_layers)
        )
    ...
    def forward(
            ...
            attention_mask: Optional[torch.Tensor] = None,
            mg_patch_mask: Optional[torch.Tensor] = None,
            mg_start_layer: int = 3,
            mg_end_layer: Optional[int] = None,
            mg_mask_threshold: float = 0.5,
            mg_neg_bias: float = -1e4,
            attention_bias_eta: float = 1.0,
            **kwargs: Unpack[TransformersKwargs],
        ...
        resolved_end = len(self.layers) if mg_end_layer is None else int(mg_end_layer)
        resolved_start = int(mg_start_layer)
        for layer_idx, encoder_layer in enumerate(self.layers, start=1):
            use_mg = mg_patch_mask is not None and resolved_start <= layer_idx <= resolved_end
            use_token_modulation = (
                self.token_modulation_enabled
                and self.token_modulator is not None
                and mg_patch_mask is not None
                and int(self.token_modulation_start_layer)
                <= layer_idx
                <= int(self.token_modulation_end_layer)
            )

            if use_token_modulation:
                soft_mask = mg_patch_mask.to(
                    device=hidden_states.device,
                    dtype=hidden_states.dtype,
                ).unsqueeze(-1)
                hidden_states = hidden_states + (
                    float(self.token_modulation_lambda)
                    * soft_mask
                    * self.token_modulator(hidden_states)
                )

            hidden_states = encoder_layer(
                hidden_states,
                attention_mask,
                mg_patch_mask=mg_patch_mask if use_mg else None,
                mg_mask_threshold=mg_mask_threshold,
                mg_neg_bias=mg_neg_bias,
                attention_bias_eta=attention_bias_eta,
                **kwargs,
...
class Fgclip2VisionTransformer(nn.Module):
    def __init__(self, config: Fgclip2VisionConfig):
        ...
        self.backbone_type = getattr(config, "backbone_type", "vit")
        if self.backbone_type == "convnext":
            self.convnext_pipeline = Fgclip2ConvNeXtVisionPipeline(config)
            self.embeddings = None
            self.encoder = None
        else:
            self.convnext_pipeline = None
    ...
    def forward(
        ...
        feat_level_mask = None
        self._last_feature_attention_mask = None
        self._last_feature_hw = None

        if self.backbone_type == "convnext":
            hidden_states, feat_level_mask, feat_hw = self.convnext_pipeline(
                pixel_values, attention_mask, spatial_shapes
            )
            self._last_feature_attention_mask = feat_level_mask
            self._last_feature_hw = feat_hw
            encoder_outputs = BaseModelOutput(last_hidden_state=hidden_states)
        else:
            hidden_states = self.embeddings(pixel_values, spatial_shapes)

            if attention_mask is not None and (
                    self.config._attn_implementation != "flash_attention_2" or mg_patch_mask is not None
            ):
        ...
        head_mask = feat_level_mask if self.backbone_type == "convnext" else attention_mask
        pooler_output = self.head(last_hidden_state, head_mask) if self.use_head else None
...
class Fgclip2VisionModel(Fgclip2PreTrainedModel):
    ...
    def get_input_embeddings(self) -> nn.Module:
        if self.vision_model.embeddings is not None:
            return self.vision_model.embeddings.patch_embedding
        return self.vision_model.convnext_pipeline.convnext.features[0][0]

    # 不使用 @check_model_inputs：新版 transformers 對 vision forward 的裝飾器簽名會導致
    # `unexpected keyword argument pixel_values`（與 use_cache / 簽名推斷有關）。
    def forward(
            self,
            pixel_values: torch.FloatTensor,
...
class SARCModel(Fgclip2Model):
    """Paper-aligned SARC model entry point with a frozen FG-CLIP backbone."""
